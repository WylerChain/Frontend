import { ACCOUNT_STATUS_TYPE_MAP, AccountStatusType } from "@/constants/accounts";
...
export const ROUTING_MANAGE_MAP = {
  ...
  requiredVerifyEmail: {
    ...
    allowedPaths: [...BASE_ALLOWED_PATHS, new RegExp(`^(\/auth\/required-verify-email)$`)],
 ...
 */
export const routingManageResult = (status: AccountStatusType, pathname: string) => {
  const routingManger = ROUTING_MANAGE_MAP[status];
  const isAllowed = routingManger.allowedPaths.some((allowedPath) => allowedPath.test(pathname));
  return {
    isAllowed,
