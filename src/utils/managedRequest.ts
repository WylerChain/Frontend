...
export const isLoadingState = <T>(data: RequestManageState<T>): data is Loading => {
  return !isInitState(data) && data.isLoading;
};

export const isSuccessState = <T>(data: RequestManageState<T>): data is Success<T> => {
...
export const isFailState = <T>(data: RequestManageState<T>): data is Fail => {
  return !isInitState(data) && !isLoadingState(data) && data.error !== undefined;
};
